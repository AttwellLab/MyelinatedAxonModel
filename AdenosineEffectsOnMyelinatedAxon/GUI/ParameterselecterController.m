classdef ParameterselecterController < handle
    ...
    methods
        ...
        function StimulationDelayUpdate(obj, hObj)
            
            value = str2double(get(hObj, 'String'));
            if CheckValue(value, 'real', [eps, inf])
                set(hObj, 'String', num2str(obj.model.stim.delay.value))
                error('Delay invalid')
            end
            obj.model.stim.delay.value = value;
        end
        ...
        function RestingPotentialUpdate(obj, hObj)
            ...
            obj.model = CalculateLeakConductanceNoIh(obj.model);
      ...
      %          error('ERev invalid')
      %      end
       %     obj.model = UpdateLeakReversal(obj.model, value);
        %    obj.model = CalculateLeakConductanceNoIh(obj.model);
         %   obj.view.InitializeGUI();
       % end
        ...
        function RunModel(obj)
            ...
            [mp, il, t] = Model_DELAY(obj.model, fullfilename);
            ...
            if get(obj.view.handles.printCVCheck, 'Value')
                ...
                %modified 2019/10/04 for delay velocity
                veldelay = velocitiesdelay(mp, il, t(2)-t(1), obj.options.cvNodes, obj.model.stim.delay.value, obj.options.cvMethod, obj.options.voltageCross);
                fprintf('Conduction velocity with delay: %.8f m/s\n', veldelay);
        ...
        function SaveModel(obj)
            ...
            save(fullfile(pathname, filename), 'axon');
        ...
        function LoadModel(obj)
            ...
            if ~isfield(axon.stim,'delay')
                axon.stim.delay.value = 0;
                axon.stim.delay.units = {1,'ms',1};
            end
