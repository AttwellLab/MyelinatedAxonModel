 function par = CalculateLeakConductanceNoIh(par)
...
%identify HAC in par
for i = 1 : length(par.node.elec.act)
    tf(i) = contains(par.node.elec.act(i).channames, "Ado");
end
idx = find(tf==1);
% Sum of active channel currents at resting membrane potential BUT don't include HAC.
activesum = zeros(par.geo.nnode, par.geo.nnodeseg);
for j = setdiff(1 : length(par.node.elec.act) , idx)
