function par = UpdateInternodeSegmentDiameter(par, value, internodeIdx, segmentIdx, myelinUpdate)
%%UPDATEINTERNODESEGMENTDIAMETER
%   par = UPDATEINTERNODESEGMENTDIAMETER(par, value, [internodeIdx], [segmentIdx], [myelinUpdate])
%       Outputs:
...
%   Examples:
%
%       UpdateInternodeSegmentDiameter(par, 1)
%           set all segments in all internodes to have diameter 1.
%
%       UpdateInternodeSegmentDiameter(par, 1, 3)
%           set all segments in the 3rd internode to have diameter 1.
%
%       UpdateInternodeSegmentDiameter(par, 1, [], 3)
%           set the 3rd segment of all internodes to have diameter 1.
%
%       UpdateInternodeSegmentDiameter(par, [1, 2], [3, 4])
%           set all segments in the 3rd internode to diameter 1, and all
%           segments in the 4th internode to diameter 2.
%
%       UpdateInternodeSegmentDiameter(par, [1, 2], [], [3, 4])
%           set 3rd segment in the all internodes to diameter 1, and 4th
%           segment in all internodes to diameter 2.
%
%       UpdateInternodeSegmentDiameter(par, [1, 2, 3, 4, 5, 6], [1, 1, 1, 5, 5, 5], [1, 3, 5, 2, 4, 6])
...
idx = sub2ind(size(par.intn.seg.geo.diam.value.vec), internodeIdx2, segmentIdx2);
par.intn.seg.geo.diam.value.vec(idx) = value;

% Derive reference values from this.
par.intn.seg.geo.diam.value.ref = mode(par.intn.seg.geo.diam.value.vec(:));
par.intn.geo.diam.value.vec = mode(par.intn.seg.geo.diam.value.vec, 2);
par.intn.geo.diam.value.ref = mode(par.intn.geo.diam.value.vec);
