%Written by Lorena Arancibia 2021-02-02
function par = UpdateLeakReversal(par, value, nodeIdx, segmentIdx)
%%UPDATELEAKREVERSAL
%   par = UPDATELEAKREVERSAL(par, value, [nodeIdx], [segmentIdx])
%       Outputs:
%               par -   modified parameter structure
%       Inputs:
%               par -   existing parameter structure
%               value -     the ERev to insert into the node or node segments
%
...
%   Examples:
%
%       UpdateLeakReversal(par, 1)
%           set the total length of all nodes to 1.
%
%       UpdateLeakReversal(par, 1, 3)
%           set 3rd node to have length 1.
%
%       UpdateLeakReversal(par, 1, [], 3)
%           set the 3rd segment of all nodes to have erev 1.
%           
%
%       UpdateLeakReversal(par, [1, 2], [3, 4])
%           set the 3rd node to have total erev 1, and the 4th node to have total erev 2.
%
%       UpdateLeakReversal(par, [1, 2], [], [3, 4])
%           set 3rd segment in the all nodes to erev 1, and 4th
%           segment in all nodes to length 2.
%
%       UpdateLeakReversal(par, [1, 2, 3, 4, 5, 6], [1, 1, 1, 5, 5, 5], [1, 3, 5, 2, 4, 6])
%           set the 1st, 3rd and 5th segment of the 1st node to erevs 1, 2, and 3, respectively,
%           and the 2nd, 4th and 6th segment of the 5th node to erevs 4, 5, and 6, respectively.
%
...
par.node.elec.pas.leak.erev.value.vec(idx) = value;
par.node.elec.pas.leak.erev.value.ref = mode(par.node.elec.pas.leak.erev.value.vec(:));